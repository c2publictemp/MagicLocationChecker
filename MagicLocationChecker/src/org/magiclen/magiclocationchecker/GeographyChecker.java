...
public class GeographyChecker extends LocationChecker2D {
	...
	public static class Vertex extends LocationChecker2D.Vertex {
		...
		public static double computeDistance(final LocationChecker2D.Vertex from, final LocationChecker2D.Vertex to) {
			...
			do {
				...
				lambda = L + (1.0 - C) * f * sinAlpha
						* (sigma + C * sinSigma * (cos2SigmaM + C * cosSigma * (-1.0 + 2.0 * cos2SigmaM2)));
			...
			final double deltaSigma = B * sinSigma * (cos2SigmaM + B / 4.0 * (cosSigma * (-1.0 + 2.0 * cos2SigmaM2)
					- B / 6.0 * cos2SigmaM * (-3.0 + 4.0 * sinSigma2) * (-3.0 + 4.0 * cos2SigmaM2)));
		 ...
		 *            傳入緯度
		 */
		public Vertex(final double longitude, final double latitude) {
