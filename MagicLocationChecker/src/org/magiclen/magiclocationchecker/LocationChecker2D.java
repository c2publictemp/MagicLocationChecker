...
public class LocationChecker2D {
	public LocationChecker2D(Vertex[] v){
		this.areaVertices=v;
	}
	// -----類別常數-----
	/**
	...
	public LocationChecker2D(final boolean autoCCW, final Vertex... vertices) {
		...
		for (final Vertex vertex : vertices) {
			...
			if (downRight == null || doubleEquals(downRight.y, vertex.y) && downRight.x < vertex.x
					|| downRight.y > vertex.y) {
				downRight = vertex;
			}
		...
		if (autoCCW) {
			...
			for (int i = 1; i < verticesSize; ++i) {
				...
				final Vertex vertex = verticesList.get(index);
				topDown.add(
						new VertexWithInfo(index, vertex, upLeft.computeAngle(vertex), upLeft.computeDistance(vertex)));
			}
			topDown.sort(null);
	...
	public boolean isInTheLocation(final Vertex vertex) {
		...
		for (i = 0, j = areaVerticesLength - 1; i < areaVerticesLength; j = i++) {
			...
			if (((v1.y >= vertex.y) != (v2.y >= vertex.y))
					&& (vertex.x <= (v2.x - v1.x) * (vertex.y - v1.y) / (v2.y - v1.y) + v1.x)) {
				c = !c;
			}
		}
		return c;
